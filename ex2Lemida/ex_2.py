...
def main():
    ...
    result_knn = knn(x_train, x_test, y_train, 7)
    result_perceptron = perceptron(x_train, x_test, y_train)
    result_pa = pa(x_train, x_test, y_train)

    len = x_test.shape[0]
    for i in range(len):
        print(f"knn: {result_knn[i]}, perceptron: {result_perceptron[i]}, pa: {result_pa[i]}")
